...
import { verifyToken, register, login, signToken } from '../auth';
...
UserRouter.get('/:id', verifyToken, (req, res, next) => {
    UserModel.findOne({ _id: req.params.id }, { pwd: 0, creationdate: 0, __v: 0 }, (err, user) => {
 ...
 *          - signToken
 */
UserRouter.post('/create', register, signToken);
 ...
 */
UserRouter.patch('/:id', verifyToken, (req, res, next) => {
    const _ = req.body;

    if (!_) return res.status(406).json({
        status: 406,
        msg: 'No user data provided to be updated.',
        user: null
    });
    UserModel.findById(req.params.id, (err, user) => {
        ...
        if (_.name) user.set('name', _.name);
        if (_.birthday) {
            req.bday = new Date();
            req.bday.setTime(_.bday)
            user.set('birthday', req.bday);
        }
        if (_.desc) user.set('description', _.desc);
        user.save()
 ...
 */
UserRouter.delete('/terminate/:id', verifyToken, (req, res, next) => {
